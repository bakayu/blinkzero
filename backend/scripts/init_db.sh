...
# Check if a database name has been set, otherwise default to 'blinkzero'
DB_NAME=${POSTGRES_DB:=blinkzero}
...
if [[ -z "${SKIP_DOCKER}" ]]
then
  # Force remove the old container if it exists to avoid conflict
  docker rm -f velo-postgres || true
